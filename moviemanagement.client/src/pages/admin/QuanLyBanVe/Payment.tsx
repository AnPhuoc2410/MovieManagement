...
const Payment: React.FC = () => {
  ...
  const handleBack = async () => {
    if (connection && selectedSeatsInfo.length > 0) {
      try {
        ...
        toast.error(t("toast.error.seat.error.booking"));
        navigate(-1);
      }
    } else {
      navigate(-1);
  ...
  const handleSearchUser = async () => {
    if (!userSearchInput.trim()) {
      toast.error(t("toast.error.user.validate"));
      return;
    }
    ...
    try {
      ...
      if (!data) {
        // No user found
        setSelectedUser(null);
        toast.error(t("toast.error.user.cannot_find_member"));
        return;
      }
      ...
      if (Array.isArray(data)) {
        ...
        if (data.length === 1) {
          ...
          setSelectedUser(null);
          toast.success(`${data.length} ${t("toast.success.user.answer")}`);
        }
      } else {
        // Single user object
      ...
      toast.error(t("toast.error.user.cannot_find"));
      setSelectedUser(null);
    } finally {
    ...
    try {
      ...
      if (selectedUser) {
        ...
        if (response.status === 200) {
          toast.success(t("toast.success.payment.success"));
      ...
      toast.error(t("toast.error.payment.error"));
    } finally {
      setIsPaying(false);
    }
  };
              ...
              >
                ...
                >
                  <Typography variant="h5" fontWeight="bold">
                    {t("admin.ticket_management.payment")}
                  </Typography>
                </Box>
                  ...
                  <Grid container spacing={3}>
                      ...
                      >
                        ...
                        <Box sx={{ mt: 2 }}>
                          <Typography variant="body1" gutterBottom>
                            <strong>{t("payment.movie.name")}</strong> {roomName}
                          </Typography>
                          <Typography variant="body1" gutterBottom>
                          ...
                          <Typography variant="body1" gutterBottom>
                            <strong>{t("movie_seat.selection")}</strong>{" "}
                            {seats.join(", ") || t("admin.ticket_management.not_selection_ticket")}
                          </Typography>
                        </Box>
                      </Paper>
                    </Grid>
                    ...
                    <Grid item xs={12} md={8}>
                        ...
                        {selectedUser ? (
                          ...
                          >
                            ...
                            >
                                ...
                                <Grid container spacing={1}>
                                  <Grid item xs={12} sm={6}>
                                    <Typography variant="body2">
                                      <strong>{t("auth.signup.fields.email")}</strong>{" "}
                                      {selectedUser.email}
                                    </Typography>
                                    <Typography variant="body2">
                                      <strong>{t("auth.signup.fields.phone")}</strong>{" "}
                                      {selectedUser.phoneNumber}
                                    </Typography>
                                  </Grid>
                                  <Grid item xs={12} sm={6}>
                                    <Typography variant="body2">
                                      <strong>{t("auth.signup.fields.id_card")}</strong>{" "}
                                      {selectedUser.idCard}
                                    </Typography>
                                    <Typography variant="body2">
                                      <strong>{t("auth.signup.fields.fullname")}</strong>{" "}
                                      {selectedUser.userName}
                                    </Typography>
                                  </Grid>
                                </Grid>
                              ...
                              <Chip
                                label={`${selectedUser.point} ${t("common.table_header.user.points")}`}
                                color="primary"
                                sx={{ ml: "auto" }}
                              />
                            </Box>
                          ...
                          <Alert severity="info" sx={{ mt: 2 }}>
                            {t("alert.ticket_management.description")}
                          </Alert>
                        )}
                      </Paper>
                      ...
                      <Paper elevation={1} sx={{ p: 3 }}>
                        <Typography variant="h6" gutterBottom>
                          {t("admin.ticket_management.payment_infor")}
                        </Typography>

                        <Box sx={{ display: "flex", justifyContent: "space-between", mb: 1 }}>
                          <Typography variant="body1">{t("payment.total_amount")}</Typography>
                          <Typography variant="body1" fontWeight="bold">
